...
module SwaggerClient
  class OauthApi
    ...
    # @return [OAuth2Token]
    def post_oauth2_token(user_key, authorization, grant_type, scope, opts = {})
      data, _status_code, _headers = post_oauth2_token_with_http_info(user_key, authorization, grant_type, scope, opts)
      return data
    end
    ...
    def post_oauth2_token_with_http_info(user_key, authorization, grant_type, scope, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: OauthApi.post_oauth2_token ..."
      end
      # verify the required parameter 'user_key' is set
      fail ArgumentError, "Missing the required parameter 'user_key' when calling OauthApi.post_oauth2_token" if user_key.nil?
      # verify the required parameter 'authorization' is set
      fail ArgumentError, "Missing the required parameter 'authorization' when calling OauthApi.post_oauth2_token" if authorization.nil?
      # verify the required parameter 'grant_type' is set
      fail ArgumentError, "Missing the required parameter 'grant_type' when calling OauthApi.post_oauth2_token" if grant_type.nil?
      # verify the required parameter 'scope' is set
      fail ArgumentError, "Missing the required parameter 'scope' when calling OauthApi.post_oauth2_token" if scope.nil?
      # resource path
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: OauthApi#post_oauth2_token\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
